...
class NC2VTK:
    def __init__(self, nc_file_path, msk_file_path):
        ...
        self.msk_input = ncw.NCFile(msk_file_path)
    ...
    def _nc2vtk_center(self, var:ncw.NCVars, use_filter:bool):
        ...
        msk = self.msk_input.var_msk_data(var).flatten()
        if use_filter:
            lon = lon[msk == 0]
            lat = lat[msk == 0]

        cart_points = cv.convert_data_arrays(cv.Mode2DTO3D, lon, lat, np.zeros((len(lon),)), len(lon))
    ...
    def _nc2vtk_corner(self, var:ncw.NCVars, use_filter:bool):
        ...
        if use_filter: clo_cla_points = clo_cla_points[:, msk == 0, :]
